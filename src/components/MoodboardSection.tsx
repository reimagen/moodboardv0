...
const MoodboardSection: React.FC = () => {
  const { moodboard, editingIndex, editPrompt, setEditPrompt, setEditingIndex, handleRemix, handleRemoveMoodboardItem } = useGeneration();
  const { summary } = useAestheticGuideState();
  const { step } = useSharedState();

  const title = step === AppStep.MOODBOARD ? "Aesthetics Guide" : "Brand Moodboard";
                ...
                {editingIndex === i ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleRemix(i)} className="flex-1 bg-blue-600 text-white text-[10px] py-1 rounded font-bold uppercase">
                        Update
                      </button>
                      <button
